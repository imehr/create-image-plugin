...
export class TemplateLoader {
  ...
  constructor(repositoryPath: string, cacheTTL: number = 3600000) {
   ...
   */
  async load(templateName: string): Promise<Template | null> {
    // Check cache first
   ...
   */
  private async loadFromDisk(templateName: string): Promise<Template | null> {
    ...
    try {
      ...
      let styleGridPath: string | undefined;
  ...
  async loadRegistry(): Promise<TemplateRegistry> {
    ...
    if (fs.existsSync(this.registryPath)) {
      try {
        const loaded = JSON.parse(fs.readFileSync(this.registryPath, 'utf-8')) as TemplateRegistry;
   ...
   */
  private async buildRegistry(): Promise<TemplateRegistry> {
   ...
   */
  private saveRegistry(registry: TemplateRegistry): void {
    try {
   ...
   */
  async findByTopic(topic: string): Promise<TemplateMetadata[]> {
    const registry = await this.loadRegistry();
   ...
   */
  async findByTag(tag: string): Promise<TemplateMetadata[]> {
    const registry = await this.loadRegistry();
   ...
   */
  async search(keyword: string): Promise<TemplateMetadata[]> {
    const registry = await this.loadRegistry();
   ...
   */
  async rebuildRegistry(): Promise<TemplateRegistry> {
